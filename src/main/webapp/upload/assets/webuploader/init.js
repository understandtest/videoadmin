jQuery(function() {
    ...
    $.fn.webupload_fileinput = function (config) {
        ...
        function init () {
            ...
            function addFile( file ) {
                var $li = $( '<li id="' + file.id + '">' +
                    ...
                    '<p class="imgWrap" style="margin: 0"></p>'+
                    '</li>' ),
                        ...
                        if ( stats.successNum ) {
                            if (uploadedFiles.length > 0) {
                                if (!config.pick.multiple) {
                                    ...
                                    var id = chooseObject.parent().prev().attr('id');
                                    var multierr = false;
                                    $.each(uploadedFiles, function(idx, url) {
                                        let imgUrl = url._raw.split("|")[1];
                                        if (imgUrl) {
                                            chooseObject.parent().parent().next().append('<div class="multi-item"><img src="'+imgUrl +'" class="img-responsive img-thumbnail cus-img" data-toggle="modal" data-target="#select_image"><em class="close delMultiImage" title="删除这张图片">×</em></div>');
                                        } else {
                                            if (!multierr) {
                                                multierr = true;
                                                alert(url.msg);
        ...
        $(".multi-img-details").delegate(".delMultiImage", "click", function () {
            $(this).parent().remove();
        });
