...
layui.define(['layer', 'laytpl', 'upload'], function(exports){
  ...
  LAYIM.prototype.initPopChat = function(){
	  return initPopChat(),this;
  }
  ...
  var elemChatTpl = ['<div class="layim-chat layim-chat-{{d.data.type}}">'
    ,'<div class="layim-chat-title">'
      ,'<a class="layim-chat-other">'
        ,'<img src="{{ d.data.avatar }}"><span layim-event="{{ d.data.type==="group" ? \"groupMembers\" : \"\" }}">{{ d.data.name||d.data.username||"佚名" }} {{d.data.temporary ? "<cite>临时会话</cite>" : ""}} {{# if(d.data.type==="group"){ }} <em class="layim-chat-members"></em><i class="layui-icon">&#xe61a;</i> {{# } }}</span>'
      ,'</a>'
    ,'</div>'
    ,'{{# if(d.data.order){ }}'
    ,'<div style="background:#e4effe;padding:7px 2%;text-align:center;">'
      ,'<a style="color:#2196f9"  href="/order/detailFirst?orderNumber={{d.data.order}}">'
        ,'<span>订单号:{{ d.data.orderShowNumber }}</span>'
      ,'</a>'
    ,'</div>'
    ,'{{# }; }}'
    ,'{{# if(d.data.adserviceId){ }}'
    ,'<div style="background:#e4effe;padding:7px 2%;text-align:center;">'
        ,'{{# if(d.data.adserviceType==1){ }}'
          ,'<a style="color:#2196f9"  href="/servicePrice/detail?id={{d.data.adserviceId}}">'
          	,'<span>预购服务名称:{{ d.data.adserviceName}}</span>'
          ,'</a>'
        ,'{{# }; }}'
        ,'{{# if(d.data.adserviceType==2){ }}'
        	,'<a style="color:#2196f9"  href="/tAdservicePackage/detail?id={{d.data.adserviceId}}">'
                ,'<span>预购服务名称:{{ d.data.adserviceName}}</span>'
            ,'</a>'
          ,'{{# }; }}'
    ,'</div>'
  ...
  layui.data.content = function(content){
    ...
    .replace(/\[([^\s\[\]]+?)\]/g, function(face){  //转义表情   //replace(/face\[([^\s\[\]]+?)\]/g, function(face)
      var alt = face.replace('&#39;', '\'');    //var alt = face.replace(/^face/g, '');
  ...
  var elemChatList = '<li class="layim-chatlist-{{ d.data.type }}{{ d.data.id }} layim-this" layim-event="tabChat"><img src="{{ d.data.avatar }}"><span>{{ d.data.name||d.data.username||"佚名" }}</span>{{# if(!d.base.brief){ }}<i class="layui-icon" layim-event="closeChat">&#x1007;</i>{{# } }}</li>';
...
//    };
  ...
  var layimClose, popmin = function(content){
    ...
    return layer.open({
      ...
      ,offset: 'rb'
      ,resize: false
      ,content: '<img src="'+ (cache.mine.avatar||(layui.cache.dir+'css/pc/layim/skin/logo.jpg')) +'"><span>'+ (content||cache.base.title) +'</span>'
  ...
  //初始化历史数据
  var initPopChat = function()
  {
	  var local = layui.data('layim')[cache.mine.id] || {};
	  if(local)
	  {
		  //console.log("缓存信息:"+JSON.stringify(local.chatlog));

		  layui.each(local.chatlog, function(index, item){
	          //  popchat(item);
			 // console.log("缓存信息"+index+":"+JSON.stringify(item));
			  layui.each(item,function(index2,item2){
				 // console.log("缓存信息"+item2.content+":"+JSON.stringify(item));
				  popchat(item2);
			  });
	      })
	      //将界面缩小
  ...
  //设置聊天窗口最小化 newMsg 为空 & 新消息提醒  newMsg 有值
  var setChatMin = function(newMsg){
    ...
    layer.open({
      ...
      ,content: '<span><i class="layui-icon">&#xe63a;</i></span>'
      ,success: function(layero, index){
        if(!newMsg){
          layimMin = layero;
        }
  ...
  var faces = function(){
    ...
    layui.each(alt, function(index, item){
      var i=index+1;
      arr[item] = layui.cache.dir + 'images/faces/ee_'+ i + '.png';
    });
    return arr;
  }();
  ...
  var anim = 'layui-anim-up', events = {
    ...
    ,image: function(othis){
      ...
      layui.upload({
        ...
        ,type: type
        ,msg:upload.msg
        ,apiOldUrl:upload.imOldurl
        ,imConn:upload.imConn
        ,imfileup:upload.imfileup
        ,imgetfile:upload.imgetfile
        ,toId:thatChat.data.id
        ,success: function(res){
          if(res.code == 0){
            if(type === 'images'){
              //focusInsert(thatChat.textarea[0], 'img['+ (res.url||'') +']');
              var data = {
            	       username: cache.mine ? cache.mine.username : '访客'
            	      ,avatar: cache.mine ? cache.mine.avatar : (layui.cache.dir+'css/pc/layim/skin/logo.jpg')
            	      ,id: cache.mine ? cache.mine.id : null
