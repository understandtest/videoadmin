...
layui.define("jquery", function(e) {
			...
			i.prototype.init = function(e) {
					...
					//	console.log(id);
						...
						var l = n.children && n.children.length > 0,
							...
							s = o(
									...
									function() {
										return l ? '<i class="layui-icon layui-tree-spread">' + (n.spread ? t.arrow[1] : t.arrow[0]) + "</i>" : '<i style="margin-left:16px;"></i>';
									}(), 
									function() {
										//如果用户选择的风格是checkbox则生成checkbox
										return i.check&&i.check=="checkbox" ? '<input type="checkbox" '+((n.uncorr&&n.uncorr==true) ? 'uncorr="1"' : 'uncorr="0"')+' name="'+i.checkboxName+'" '+((n.checked&&n.checked==true) ? 'checked="checked"' : "")+(n.checkboxValue ? ('value="' + n.checkboxValue + '"') : "") + 'data-parent-id="' + n.pid +'"' + 'id="' + n.id + '"' +(i.checkboxStyle ? ('style="' + i.checkboxStyle + '"') : "" )+' />' : "";
									}(), 
									function() {
				...
				i.prototype.cancelParentsCheckboxCheck = function (ele){
					...
					//自定义的属性 uncorr  值为0和1   为0时 取消选中关联父级选中  为1 时取消选中不关联父级选中
					var uncorr = ele.getAttribute("uncorr");
					if(uncorr==0){
						var r = this,
