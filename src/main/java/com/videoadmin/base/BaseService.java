...
public abstract class BaseService<T extends BaseModel> implements ApplicationContextAware  {
	...
	public static Page<Integer> getPage(Map<String, Object> params) {
		...
		if (DataUtil.isNotEmpty(params.get("order"))){
	    	if("desc".equals(params.get("order").toString())){
	    		page.setAsc(false);
	    	}else if("asc".equals(params.get("order").toString())){
	    		page.setAsc(true);
	    	}
	    }else{
	    	//为空默认desc
	...
	/** 分页查询 */
    public  Page<Map<String, Object>> getPageMap(Map<String, Object> params) {
        Integer current = 1;
        Integer size = 10;

		String sizeStr = (String) params.get("size");

		if(StringUtils.isNotBlank(sizeStr) && !"null".equals(sizeStr)){
	...
	/** 根据Id查询(默认类型T) */
	public Page<T> getPage(final Page<Integer> ids) {
		if (ids != null) {
	...
	/** 根据Id查询(默认类型T) */
	public Page<Map<String, Object>> getPageMap(final Page<Integer> ids) {
		if (ids != null) {
	...
	/** 根据Id查询(cls返回类型Class) */
	public <K> Page<K> getPage(final Page<Integer> ids, final Class<K> cls) {
		if (ids != null) {
	...
	/** 根据Id查询(默认类型T) */
	public List<T> getList(final List<Integer> ids) {
	...
	/** 根据Id查询(cls返回类型Class) */
	public <K> List<K> getList(final List<Integer> ids, final Class<K> cls) {
	...
	@SuppressWarnings("unchecked")
	private T queryById(Integer id, int times) {
	...
	protected String getCacheKey(Object id) {
		...
		return new StringBuilder(SysConstants.CACHE_NAMESPACE).append(cacheName).append(":").append(id).toString();
	}

	/** 获取缓存键值 */
	protected String getLockKey(Object id) {
		String cacheName = getCacheKey();
		return new StringBuilder(SysConstants.CACHE_NAMESPACE).append(cacheName).append(":LOCK:").append(id).toString();
	}
	...
	private String getCacheKey() {
		Class<?> cls = getClass();
		String cacheName = SysConstants.cacheKeyMap.get(cls);
		if (StringUtils.isBlank(cacheName)) {
			...
			SysConstants.cacheKeyMap.put(cls, cacheName);
		}
		return cacheName;
	}
